        ...
        if response.status_code == 200:
            ...
            if "www.ncbi.nlm.nih.gov" in response.url:
                ...
                result = quintessence_publishing(
                    doi_soup, output_folder, filename, headers
                )
                return result
            elif "pubs.rsc.org" in response.url:
                result = pubs_rsc_org(
                    response, doi_soup, output_folder, filename, headers
                )
                return result
            elif "academic.oup.com" in response.url:
                result = academic_oup_com(
                    response, doi_soup, output_folder, filename, headers
                )
                return result
                ...
                try:
                    ...
                    return False
                doi_soup = BeautifulSoup(response.content, "html.parser")
                result = science_direct(
                    response, doi_soup, output_folder, filename, headers
                )
                return result
            elif "link.springer.com" in response.url:
                result = springer_link(doi_soup, output_folder, filename, headers)
                return result
            elif "bmcoralhealth.biomedcentral.com" in response.url:
                result = bmcoralhealth_biomedcentral(
                    response, doi_soup, output_folder, filename, headers
                )
                return result
...
def academic_oup_com(response, doi_soup, output_folder, filename, headers):
    ...
    if currency_wrap:
        ...
        pdf_link = doi_soup.find(
            "a",
            class_="get-access get-access-jumplink at-get-access-jumplink js-no-access-jumplink",
        )
        if pdf_link:
...
def science_direct(response, doi_soup, output_folder, filename, headers):
    access_link = doi_soup.find(
        "li",
        class_="accessbar-item-hide-from-initial accessbar-item-hide-from-xs accessbar-item-show-from-md PurchasePDF",
    )
    if access_link:
        return False
    else:
        ...
        if access_link:
            pdf_url = access_link.find(
                "a",
                class_="link-button accessbar-utility-component accessbar-utility-link link-button-primary link-button-icon-left",
            )
...
def medscimonit(doi_soup, output_folder, filename, headers):
    ...
    if pdf_link:
        ...
        payload = {"ID_JOUR": id_jour, "idArt": id_art}
        ...
        if response_post.status_code == 200:
            ...
            print(
                f"Error: PDF download failed for {filename}. Status code: {response_post.status_code}"
            )
            return False
    else:
        return False
...
def swiss_dental_journal(doi_soup, output_folder, filename, headers):
    ...
    if pdf_link:
        ...
        if new_pdf_response.status_code == 200:
            ...
            if download:
                ...
                if download_url_response.status_code == 200:
                    write_result = write_pdf(
                        download_url_response, output_folder, filename
                    )
                    return write_result
                else:
                    print(
                        f"Error: PDF download failed for {filename}. Status code: {download_url_response.status_code}"
                    )
                    return False
            else:
                return False
            ...
            print(
                f"Error: PDF download failed for {filename}. Status code: {new_pdf_response.status_code}"
            )
            return False
    else:
        return False
